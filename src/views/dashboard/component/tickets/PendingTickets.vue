<template>
  ...
  >
    ...
    >
      ...
      >
        <base-material-card
          :title="$t('confirmSolveTicket')"
          color="success"
        >
          <v-card-text class="mt-8">
            <p>{{ $t('areYouSureToMove') }} {{ ticketName }} {{ $t('toSolvedList') }}</p>
          </v-card-text>
    ...
    >
      {{ $t('thisTicketIsMovedToSolvedListSuccssfly') }}
    </base-material-snackbar>
    <base-material-snackbar
      v-if="snackbar3"
      v-model="snackbar3"
  ...
  export default {
    ...
    methods: {
      ...
      async fetchAllItems () {
        ...
        const tickets = await ticketsServices.getPendingTickets(itemsPerPage, page, pageNumber, this.search)
        this.tickets = tickets.data.data
        this.total = tickets.data.total
        this.numberOfPages = tickets.data.pageCount
        this.dataLoading = false
      },
      async solveTicket (tickets) {
        this.disabled = true
        this.loading = true
        const ticketId = tickets.id
        const solveSucess = await ticketsServices.solveTickets(ticketId)
        if (solveSucess) {
          this.dialog1 = false
          this.snackbar2 = true
          this.disabled = false
