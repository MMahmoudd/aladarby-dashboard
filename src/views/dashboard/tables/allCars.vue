  ...
  export default {
    ...
    methods: {
      ...
      async fetchAllItems () {
        ...
        const cars = await carsServices.getAllCars(itemsPerPage, page, pageNumber, this.search)
        this.cars = cars.data.data
        this.totalCars = cars.data.total
        this.numberOfPages = cars.data.pageCount
        this.dataLoading = false
      },
      async blockCar (carsData) {
        this.disabled = true
        this.loading = true
        this.carsData = carsData
        const carId = carsData.id
        const carsUpdatesucess = await carsServices.blockCar(carId)
        if (carsUpdatesucess) {
          this.snackbar1 = true
          this.dialog3 = false
          this.disabled = false
          this.loading = false
          this.$router.go('/all-cars')
        } else {
          this.snackbar2 = true
          this.disabled = false
          this.loading = false
